<?php
if(!class_exists("BarsiBase")) {
	class BarsiBase {
    	public $key = "24A4F688EB7D85A8";
    	private $product_id = "1";
    	private $product_base = "barsi-license";
    	private $server_host = "https://themexriver.com/wp/barsi/wp-json/licensor/";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_barsi-license_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=barsi-license_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
