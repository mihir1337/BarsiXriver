...
class Barsi_Mega_Menu_Walker extends Walker_Nav_Menu
{
    private $item;
    ...
    public function start_el(&$output, $item, $depth = 0, $args = array(), $id = 0)
    {
        ...
        if (!empty($item->ct_megaprofile) && $megamenu) {
            $item->classes[] = 'tf-megamenu';
            $item->classes[] = 'megamenu-style-alt';
            $item->classes[] = 'menu-item-has-children dropdown';
        }
    ...
    public function get_megamenu($id)
    {
        ...
        $megamenu = apply_filters('ct_enable_megamenu', false);
        if ($megamenu)
            return '<ul class="sub-menu"><li><div class="container-fluid p-0">' . $content . '</div></li></ul>';
        else
            return false;
    }
